...
vscodium_restore() {
  ...
  local b
  b=$(find "${d%/*}" -maxdepth 1 -name "${d##*/}.backup.*" -printf "%T@ %p\n" | sort -rn | head -1 | cut -d' ' -f2-)
  [[ -z $b ]] && die "No backup"
  cp -f "$b" "$d" && printf '%b\n' "${G}✓ Restored $b${D}"
}
# ─── Main ───
main() {
  ...
  xdg) xdg_patch ;;
  xdg-data)
    f="${2:-/usr/share/vscodium/resources/app/product.json}"
    sed -i 's|"dataFolderName": ".*"|"dataFolderName": ".local/share/codium"|' "$f" && echo "✓ DataFolder"
    ;;
  ...
  mkt)
    apply_json "${2:-$C_P}" "${3:-$C_DIR/code-marketplace/patch.json}" "${4:-$C_DIR/code-marketplace/cache.json}"
    sign_fix node-ovsx-sign
    ;;
  mkt-restore)
    restore_json "${2:-$C_P}" "${3:-$C_DIR/code-marketplace/patch.json}" "${4:-$C_DIR/code-marketplace/cache.json}"
    sign_fix
    ;;
  mkt-update)
    K=(extensionsGallery extensionRecommendations keymapExtensionTips languageExtensionTips configBasedExtensionTips webExtensionTips virtualWorkspaceExtensionTips remoteExtensionTips extensionAllowedBadgeProviders extensionAllowedBadgeProvidersRegex msftInternalDomains linkProtectionTrustedDomains)
    update_json "${2:-}" "${3:-./patch.json}" K
    ;;
  all)
    find_files | xdg_patch
    repo_swap "" 0
    apply_json "$C_P" "$C_DIR/code-marketplace/patch.json" "$C_DIR/code-marketplace/cache.json"
    apply_json "$C_P" "$C_DIR/code-features/patch.json" "$C_DIR/code-features/cache.json"
    sign_fix node-ovsx-sign
    ;;
  all-vscodium)
    find_files | xdg_patch
