...
while getopts dhrp: opt; do
  case $opt in
    d) [[ -z $LINK ]] && detect_profile; printf 'Default: ~/.mozilla/firefox/%s\n' "$LINK"; exit 0 ;;
    h) longhelp ;;
    p) LINK=$OPTARG ;;
    r) [[ ! -d $VOLATILE ]] && { printf 'Error: Volatile dir not found: %s\n' "$VOLATILE" >&2; exit 1; }; builtin cd ~/.mozilla/firefox || exit 1; [[ -L $LINK ]] && rm "$LINK"; [[ -d $STATIC ]] && mv "$STATIC" "$LINK"; exit 0 ;;
    *) usage >&2; exit 1 ;;
  esac
done
...
builtin cd ~/.mozilla/firefox || { printf 'Error: ~/.mozilla/firefox missing\n' >&2; exit 1; }
