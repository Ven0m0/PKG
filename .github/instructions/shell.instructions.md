...
- Prefer shared helpers such as `lib/helpers.sh` and `find_pkgbuilds` when they already cover the job
...
Use `pkg.sh lint` for repo-level validation when shell changes touch package automation.
